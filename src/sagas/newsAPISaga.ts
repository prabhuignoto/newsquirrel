...
export function* watchCanLoadUrlInFrame() {
  yield takeEvery([Constants.CAN_LOAD_URL_IN_IFRAME], function* canLoadUrlInFrame(action: any){
    const {url, id} = action;
    try {
      const response: AxiosResponse = yield axios.post('http://localhost:1337/canBeLoaded', {
        id,
        url,
      });
